  ...
  return docInfo && (
    <div>
      ...
      <div className="flex flex-col md:flex-row items-start gap-5 my-5">
        ...
        <div>
          <img className='bg-blue-500 w-60 rounded-lg' src={docInfo?.image} alt="" />
        </div>

        <div className='flex-1 border border-gray-300 rounded-lg p-8 py-7 bg-white sm:mx-0 sm:mt-0'>

          {/*------------------Doctor Info : name, speciality, experience, fees------------------*/}

          <p className='flex items-center gap-2 text-2xl front-medium text-gray-900'>
            {docInfo?.name}
      ...
      <div className='sm:ml-72 sm:pl-4 font-medium text-gray-700'>
        ...
        <div className='flex item-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].slots.map((item, index) => (
            <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-blue-500 text-white' : 'border border-gray-300'}`} key={index}>
              {item.time.toLowerCase()}
            </p>
          ))}
        </div>
        <button onClick={bookAppointment} className='bg-blue-500 text-white text-sm font-light px-14 py-3 rounded-full my-6 cursor-pointer'> Book an Appointmet</button>
      </div>
