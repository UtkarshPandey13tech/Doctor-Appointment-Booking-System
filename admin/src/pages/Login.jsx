...
const Login = () => {
    ...
    const onSubmitHandler = async (event) => {
         event.preventDefault() //not reload the web page
...
<div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
    ...
    <div className='w-full'>
        <p>Email</p>
        <input onChange={(e) => setEmail(e.target.value)} value={email} className='border border-b-black rounded w-full p-2 mt-1' type="email" required />
    </div>
    <div className='w-full'>
        <p>Password</p>
        <input onChange={(e) => setPassword(e.target.value)} value={password}  className='border border-b-black rounded w-full p-2 mt-1' type="password" required />
    </div>
